...
if __name__ == "__main__":
    ...
    with open("data/warriors_roster.json", "r") as f:
        roster = json.load(f)

    shutil.copy("data/roster.ros", "data/warriors_poc.ros")

    reader = RosReader("data/warriors_poc.ros")
    writer = RosWriter("data/warriors_poc.ros")
    
    print("\\n=== WARRIORS TRANSLATION MATRIX STARTING ===")

    for player_name, player_id in roster.items():
        print(f"\\nProcessing: {player_name}")
        player_data = client.get_player_data(player_id, player_name, "2024-25")
        ros_values = matrix.translate_player(player_data)

        try:
            record = reader.find_player_by_name(player_name)
        except ValueError:
            print(f"[{player_name}] Not found in roster binary. Skipping.")
            continue

        for field_name, tier_value in ros_values.items():
            if "_" not in field_name or field_name.startswith("hz_"):
                writer.write_field(record.record_index, record.sub_idx, field_name, tier_value)

        original = RosReader("data/roster.ros").find_player_by_name(player_name)
        updated  = RosReader("data/warriors_poc.ros").find_player_by_name(player_name)
    ...
    print(f"\\nWarriors POC file saved: data/warriors_poc.ros")
